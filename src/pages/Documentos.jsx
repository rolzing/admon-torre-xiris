    ...
    <div className="space-y-6">
      ...
      {asamblea && (
        <Card className="border-accent-200 bg-gradient-to-br from-white to-accent-50/40 dark:border-accent-500/30 dark:from-navy-900 dark:to-navy-800">
          ...
          <h3 className="text-lg font-bold text-navy-900 dark:text-white">{asamblea.titulo}</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400">{formatFecha(asamblea.fecha)}</p>

          {asamblea.resumenAcuerdos && (
            <div className="mt-3">
              <p className="text-sm font-semibold text-navy-800 dark:text-navy-200 mb-2">Acuerdos principales:</p>
          ...
          {documentos.map((d) => (
            <Card key={d.id} hover>
              <div className="flex items-start gap-3">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-navy-50 text-navy-600 text-xl dark:bg-navy-800 dark:text-navy-200">
                  <IconoDocumento />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <Badge tone={tipoTone(d.tipo)}>{d.tipo}</Badge>
                    <span className="text-xs text-slate-400 dark:text-slate-500">{formatFecha(d.fecha)}</span>
                  </div>
                  <h3 className="mt-1.5 font-bold text-navy-900 dark:text-white">{d.titulo}</h3>
                  {d.descripcion && <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{d.descripcion}</p>}
                  <Button
                    variant="secondary"
